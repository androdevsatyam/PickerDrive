...
import com.hpweb.pickerdrive.databinding.ActivityFormTransportVehicleBinding;
import com.model.FormModel;

public class FormTransportVehicle extends AppCompatActivity {

    ActivityFormTransportVehicleBinding formBinding;
...
//        setContentView(R.layout.activity_form_transport_vehicle);

        formBinding = DataBindingUtil.setContentView(this, R.layout.activity_form_transport_vehicle);
        if(getSupportActionBar()!=null)
            getSupportActionBar().setTitle(Type);

        formBinding.from.setAdapter(Helper.getCities(FormTransportVehicle.this));
        formBinding.to.setAdapter(Helper.getCities(FormTransportVehicle.this));
        ...
        formBinding.submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (check()) {
                    ...
                    FormModel model=new FormModel( formBinding.name.getText().toString(), formBinding.date.getText().toString(), "", formBinding.mobile.getText().toString(), formBinding.address.getText().toString(), formBinding.company.getText().toString(), "", formBinding.capweight.getText().toString(), "", formBinding.vehicletype.getText().toString(),"","", formBinding.detail.getText().toString(), "", "", "", formBinding.from.getText().toString(), formBinding.to.getText().toString(), requirement);

                    String result=Helper.submitForm(FormTransportVehicle.this,"TRANSPORTERS_VEHICLE_Requirement",id,model);
                    ...
                    //                    helper.showProgress("Submitting...");
//                    Con.getToken().insertform("packer_vehicle", id, formBinding.name.getText().toString(), formBinding.date.getText().toString(), "", formBinding.mobile.getText().toString(), formBinding.address.getText().toString(), formBinding.company.getText().toString(), "", formBinding.capweight.getText().toString(), "", formBinding.vehicletype.getText().toString(), formBinding.detail.getText().toString(), "", "", "", formBinding.from.getText().toString(), formBinding.to.getText().toString(), requirement, new Callback<FormModel>() {
//                        @Override
//                        public void success(FormModel formModel, Response response) {
//                            helper.stopProgress();
//                            if(formModel.getStatus().equalsIgnoreCase("true")) {
//                                Helper.makeToast(FormTransportVehicle.this,"Form Submitted", Toast.LENGTH_SHORT);
//                                onBackPressed();
//                            }else{
//                                Helper.makeToast(FormTransportVehicle.this,formModel.getMsg(), Toast.LENGTH_SHORT);
//                            }
//                        }
//
//                        @Override
