...
import com.hpweb.pickerdrive.databinding.ActivityFormpackercarCarrierBinding;
import com.model.FormModel;

public class FormPackerCarCarrier extends AppCompatActivity {

    ActivityFormpackercarCarrierBinding formBinding;
...
//        setContentView(R.layout.activity_form_car_carrier);

        formBinding = DataBindingUtil.setContentView(this, R.layout.activity_formpackercar_carrier);
        if(getSupportActionBar()!=null)
            getSupportActionBar().setTitle(Type);

        formBinding.from.setAdapter(Helper.getCities(FormPackerCarCarrier.this));
        formBinding.to.setAdapter(Helper.getCities(FormPackerCarCarrier.this));
        ...
        formBinding.submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (check()) {
                    ...
                    FormModel model=new FormModel(formBinding.name.getText().toString(), formBinding.date.getText().toString(), "", formBinding.mobile.getText().toString(), formBinding.address.getText().toString(), formBinding.company.getText().toString(), formBinding.carname.getText().toString(), formBinding.capweight.getText().toString(), "", "","","", "", "", "", "", formBinding.from.getText().toString(), formBinding.to.getText().toString(), requirement);

                    String result=Helper.submitForm(FormPackerCarCarrier.this,"PACKERS_CAR_CARRIER_Requirement",id,model);
...
//                    helper.showProgress("Submitting...");
//                    Con.getToken().insertform("packer_vehicle", id, formBinding.name.getText().toString(), formBinding.date.getText().toString(), "", formBinding.mobile.getText().toString(), formBinding.address.getText().toString(), formBinding.company.getText().toString(), formBinding.carname.getText().toString(), formBinding.capweight.getText().toString(), "", "", "", "", "", "", formBinding.from.getText().toString(), formBinding.to.getText().toString(), requirement, new Callback<FormModel>() {
//                        @Override
//                        public void success(FormModel formModel, Response response) {
//                            helper.stopProgress();
//                            if(formModel.getStatus().equalsIgnoreCase("true")) {
//                                Helper.makeToast(FormPackerCarCarrier.this,"Form Submitted", Toast.LENGTH_SHORT);
//                                onBackPressed();
//                            }else{
//                                Helper.makeToast(FormPackerCarCarrier.this,formModel.getMsg(), Toast.LENGTH_SHORT);
//                            }
//                        }
//
//                        @Override
